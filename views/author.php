<!DOCTYPE html>
<html>
    <head>
        <meta charset="utf-8"/>
        <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no"/>
        <meta http-equiv="x-ua-compatible" content="ie=edge"/>
        <meta name="description" content="CSA Sports Management is a full-service sports management agency dedicated to providing clients and partners with assistance in contract negotiation, career + brand management services and financial planning with the utmost professionalism, reliability and transparency."/>
        <meta name="keywords" content="CSA,csa,introducing,player,sports,sports management,agency,guard,ncaa,"/>
        <meta name="copyright" content="59Develop 2019"/>
        <title>CSA Sports Management | Author</title>
        <link rel="stylesheet" type="text/css" href="css/style.css"/>
        <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.4.1/css/bootstrap.min.css" integrity="sha384-Vkoo8x4CGsO3+Hhxv8T/Q5PaXtkKtu6ug5TOeNV6gBiFeWPGFN9MuhOf23Q9Ifjh" crossorigin="anonymous"/>
        <link rel="icon" type="image/x-icon" href="img/ikona.ico" /> 
        <link rel="stylesheet" href="https://use.fontawesome.com/releases/v5.8.2/css/all.css"/>
        <link href="css/hover-min.css" rel="stylesheet"/>
        <link href="https://fonts.googleapis.com/css?family=Muli|Righteous&display=swap" rel="stylesheet"/>
        <link href="https://fonts.googleapis.com/css?family=Manrope&display=swap" rel="stylesheet">
    </head>
    <body>
        <div id="csaHeader" class="container-fluid">
            <div class="row">
                <div id="home_background" class="col-xl-4 col-lg-12 author">
                    <div class="naslov">
                        <img src="img/logoHeader.png" alt="CSA Logo" class="logoHome"/>
                        <h1>CSA Sports Management</h1>
                        <h4>A Premier Sports Agency. Creating a Legacy.</h4>
                        <p class="est">est 2019</p>
                    </div>
                </div>
                <div id="home_text" class="col-xl-8 col-lg-12">
                    <div id="hamburger">
                        <div class="ham"></div>
                        <div class="ham"></div>
                        <div class="ham"></div>
                    </div>
                    <!-- <nav class="navbar navbar-expand-lg" > -->
                        
                    <ul class="navbar-nav" id="navigacijaLinkovi">
                                <a class="nav-link hvr-underline-from-center" href="index.php">
                                    <li class="nav-item active">
                                    Home 
                                    </li>
                                </a>  
                                 
                                <a class="nav-link hvr-underline-from-center" href="services.php">
                                    <li class="nav-item active">
                                    Services 
                                    </li>
                                </a> 
                                <a class="nav-link hvr-underline-from-center" href="clients.php">
                                    <li class="nav-item active">
                                    Clients 
                                    </li>
                                </a> 
                                <a class="nav-link hvr-underline-from-center" href="news.php">
                                    <li class="nav-item active">
                                    News 
                                    </li>
                                </a> 
                                <a class="nav-link hvr-underline-from-center" href="contact.php">
                                    <li class="nav-item active">
                                    Contact 
                                    </li>
                                </a> 
                                <a class="nav-link hvr-underline-from-center" href="author.php">
                                    <li class="nav-item active">
                                    Author 
                                    </li>
                                </a> 
                            </ul>
                        
                    </nav>
                    <div class="tekst row d-flex justify-content-center">
                        <div class="col-lg-8">
                            <h2>About me</h2>
                            <hr class="naslovHr"/>
                            <p class="mt-4 mb-4"><span class="imeFirmeTekst">Hello, I'm Nikola</span>, student of web development based in Belgrade, Serbia. I’m a people-person with deep emotions and empathy, a natural storyteller. I currently study at ICT college course Internet Technologies. Outside of my commitments I work with a select freelance client base.</p>
                            <p class="mb-4">Here are a few technologies I've been working with recently:</p>
                            <ul class="authorLista mb-4"> 
                                <li>HTML &amp; CSS</li>
                                <li>JavaScript</li>
                                <li>PHP7</li>
                                <li>SQL / MySql</li>
                            </ul>
                            <p class="btnHomeContact">DOWNLOAD CV</p>
                            <div class="bordure"></div>
                        </div>
                    </div>
                    <div class="socialHome">
                        <i class="fab fa-facebook"></i>
                        <i class="fab fa-instagram my-3"></i>
                        <i class="fab fa-twitter"></i>
                    </div>
                    <footer>
                        <h4 class="text-center"><span class="font-weight-bold">FIBA License</span> No. 201900917</h4>
                        <p>Copyright © 2020 CSA. All rights reserved.</p>
                    </footer>
            </div>
        </div>
    </body>
    <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.4.1/jquery.min.js"></script>
    <script src="https://cdn.jsdelivr.net/npm/popper.js@1.16.0/dist/umd/popper.min.js" integrity="sha384-Q6E9RHvbIyZFJoft+2mJbHaEWldlvI9IOYy5n3zV9zzTtmI3UksdQRVvoxMfooAo" crossorigin="anonymous"></script>
    <script src="https://stackpath.bootstrapcdn.com/bootstrap/4.4.1/js/bootstrap.min.js" integrity="sha384-wfSDF2E50Y2D1uUdj0O3uMBJnjuUD4Ih7YwaYd1iqfktj0Uod8GCExl3Og8ifwB6" crossorigin="anonymous"></script>
    <script src="js/main.js"></script>
</html>